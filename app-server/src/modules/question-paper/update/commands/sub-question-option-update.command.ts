...
class SubQuestionOptionUpdateDto {
  @IsString()
  optionId: string;
...
export class SubQuestionOptionUpdateCommand
  implements IQuestionPaperUpdateCommand
{
  ...
  async verifyAccess(): Promise<{ isValid: boolean; message: string }> {
    try {
      ...
      const updateData = plainToInstance(SubQuestionOptionUpdateDto, this.data);
  ...
  validate(): { isValid: boolean; errors: string[] } {
    const dto = plainToInstance(SubQuestionOptionUpdateDto, this.data);
