...
export class OptionCreateCommand implements IQuestionPaperUpdateCommand {
  private data: object;
  ...
  async verifyAccess(): Promise<{ isValid: boolean; message: string }> {
    try {
      ...
      const createData = plainToInstance(OptionCreateDto, this.data);
  ...
  validate(): { isValid: boolean; errors: string[] } {
    const dto = plainToInstance(OptionCreateDto, this.data);
  ...
  async execute(): Promise<void> {
    ...
    // Get the question reference for the new option
    const question = await this.questionRepository.findOne({
      where: {
        id: createData.questionId,
        questionPaper: { id: this.questionPaperId },
